...
def rekomendasi_makanan(dataset, penyakit_input, alergi_input, AKEi, jenis_kelamin):
    ...
    for meal_id in [1, 2, 3]:  # Loop through meal times: 1 - breakfast, 2 - lunch, 3 - dinner
        ...
        dataset_mealtime = dataset[dataset['mealtimeId'] == meal_id]
...
# Gunakan kNN untuk menemukan resep yang paling sesuai
        nbrs = NearestNeighbors(n_neighbors=10, algorithm='brute', metric='cosine')
        nbrs.fit(X_mealtime_scaled)
        distances, indices = nbrs.kneighbors(nutrisi_dibutuhkan_scaled)

        # Pilih resep dengan jarak terdekat
        for i, index in enumerate(indices[0]):
            resep_terpilih = dataset_mealtime.iloc[index]
            rekomendasi_akhir = pd.concat([rekomendasi_akhir, resep_terpilih.to_frame().T])  # Ensure it's a DataFrame
            print(f"Rekomendasi untuk mealtime {meal_id}: {resep_terpilih['Nama Resep']}")
            print(f"Jarak cosine: {distances[0][i]}")
            # print(f"Alasan Pemilihan: Resep ini memiliki jarak Cosine terkecil ({distances[0][i]}) dari kebutuhan nutrisi.")
...
# # Gunakan kNN untuk menemukan resep yang paling sesuai
#         nbrs = NearestNeighbors(n_neighbors=7, algorithm='brute', metric='euclidean')
#         nbrs.fit(X_mealtime_scaled)
#         distances, indices = nbrs.kneighbors(nutrisi_dibutuhkan_scaled)

#         # Pilih resep dengan jarak terdekat dan filter berdasarkan penyakit dan alergi
#         for index in indices[0]:
#             resep_terpilih = dataset.iloc[index]
#             if all(resep_terpilih[penyakit] == 0 for penyakit in penyakit_input) and all(resep_terpilih[alergi] == 0 for alergi in alergi_input):
#                 rekomendasi_akhir = pd.concat([rekomendasi_akhir, resep_terpilih.to_frame().T])  # Ensure it's a DataFrame
#                 print(f"Rekomendasi untuk mealtime {meal_id}: {resep_terpilih['Nama Resep']}")
#                 print(f"Jarak Euclidean: {distances[0][0]}")
